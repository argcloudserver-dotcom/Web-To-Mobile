...
export async function seed() {
  ...
  const leadsData = [
    ...
    // عرض سعر
    { name: "كريم فتحي عوض",       phone: "01201234567", email: "karim.awad@gmail.com",      source: "website" as const,  status: "meeting" as const,    notes: "أُرسل له عرض سعر لشقة B-12 في زيد التجمع. ينتظر موافقة الأسرة على العرض." },
    { name: "غادة عصام الدين",     phone: "01312345678", email: "ghada.essamdin@yahoo.com",  source: "manual" as const,   status: "meeting" as const,    notes: "مستثمرة من الإسكندرية. العرض يشمل شقتين في مدينتي بخطة تقسيط 7 سنوات." },
    { name: "إسلام رجب الحسيني",   phone: "01423456789", email: "eslam.rajab@gmail.com",     source: "referral" as const, status: "meeting" as const,    notes: "سعودي مقيم بالقاهرة. عرض سعر لفيلا ببالم هيلز أكتوبر، اهتمام قوي." },
    ...
    // مكتملة
    { name: "لميس جمال الدين",      phone: "01756789012", email: "lamis.gamaldin@gmail.com",  source: "website" as const,  status: "bought" as const, notes: "أتمّت شراء شقة في هايد بارك، وحدة 14-أ، مساحة 185 متر. 5.2 مليون جنيه.", outcome: "وُقّعت العقود وسُدّد المقدم. جاري استكمال إجراءات التسجيل." },
    { name: "عبدالرحمن عزت حجازي", phone: "01867890123", email: "abdo.hagazy@gmail.com",     source: "referral" as const, status: "bought" as const, notes: "اشترى فيلا في بالم هيلز أكتوبر. 8.7 مليون جنيه كاش. عميل راقٍ.", outcome: "تسلّم المفاتيح. راضٍ جداً عن الخدمة. أحال إلينا 3 عملاء جدد." },
    { name: "داليا طه منصور",       phone: "01978901234", email: "dalia.mansour@gmail.com",   source: "social" as const,   status: "bought" as const, notes: "اشترت شاليه في ماراسي الساحل الشمالي. 9.5 مليون جنيه.", outcome: "وُقّع العقد. استلام المفاتيح مقرر يونيو 2026." },

    // خسارة
    { name: "رامي سمير الغزالي",   phone: "01089012345", email: "ramy.ghazaly@yahoo.com",    source: "import" as const,   status: "not_qualified" as const, notes: "خسرنا المنافسة مع مشروع آخر. المشكلة الأساسية كانت السعر.", outcome: "اختار مشروع بيفرلي هيلز 6 أكتوبر بسعر أقل بمليون جنيه." },
    { name: "نانسي شوقي بهنساوي",  phone: "01190123456", email: "nancy.shawky@gmail.com",    source: "campaign" as const, status: "not_qualified" as const, notes: "سافرت للخارج وأجّلت قرار الشراء لأجل غير مسمى.", outcome: "تأجيل القرار — انتقلت للعيش في كندا بصفة مؤقتة." },
  ];
  ...
  for (const lead of createdLeads) {
    ...
    const numActivities = ["bought","not_qualified","negotiation","meeting"].includes(lead.status) ? 3 : ["called","qualified"].includes(lead.status) ? 2 : 1;
  ...
  const wonLeads = createdLeads.filter(l => l.status === "bought");
  const dealData = [
