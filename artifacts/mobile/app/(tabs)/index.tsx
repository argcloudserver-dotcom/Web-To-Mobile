...
export default function HomeScreen() {
  ...
  const kpis = [
    { label: t("home.kpi_active"),    value: activeLeads.length, icon: "users" as const,       color: cr.new.bg },
    { label: t("home.kpi_won"),       value: wonLeads.length,    icon: "trending-up" as const,  color: cr.bought.bg },
    { label: t("home.kpi_due_today"), value: todayTasks.length,  icon: "check-square" as const, color: cr.called.bg },
    { label: t("home.kpi_total"),     value: myLeads.length,     icon: "bar-chart-2" as const,  color: c.accent },
  ];
    ...
    <View style={s.container}>
      <ScreenHeader
        ...
        rightElement={
          ...
          >
            <Feather name={isDark ? "sun" : "moon"} size={15} color={c.accent} />
          </TouchableOpacity>
        }
      />
      <ScrollView
      ...
      >
        ...
        <View style={s.section}>
          <View style={s.sectionHeader}>
            <Text style={s.sectionTitle}>{t("home.my_leads")}</Text>
            <TouchableOpacity onPress={() => router.push("/(tabs)/leads")} style={s.seeAllBtn}>
              <Text style={s.seeAll}>{t("home.view_all")}</Text>
          ...
          {leadsLoading ? (
            ...
            <View style={s.empty}>
              ...
              <Text style={s.emptyText}>{t("home.no_active_leads")}</Text>
            </View>
          ) : (
              ...
              return (
                ...
                >
                  ...
                  <View style={s.leadInfo}>
                    ...
                    <Text style={s.leadProject} numberOfLines={1}>{lead.projectName ?? t("home.no_project")}</Text>
                  </View>
        ...
        <View style={[s.section, { marginBottom: 100 }]}>
          <Text style={s.sectionTitle}>{t("home.todays_tasks")}</Text>
          {tasksLoading ? (
            <View style={[s.skeleton, { height: 80 }]} />
          ) : todayTasks.length === 0 ? (
            <View style={s.empty}>
              <Feather name="calendar" size={28} color={c.mutedForeground} />
              <Text style={s.emptyText}>{t("home.no_tasks_today")}</Text>
            </View>
          ) : (
                ...
                <View key={task.id} style={s.taskRow}>
                  ...
                  {done && (
                    <View style={[s.badge, { backgroundColor: `${c.success}18` }]}>
                      <Text style={[s.badgeText, { color: c.success }]}>{t("home.done_badge")}</Text>
                    </View>
                  )}
                </View>
              );
