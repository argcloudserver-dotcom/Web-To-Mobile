...
export default function ProjectsScreen() {
  ...
  return (
    <View style={s.container}>
      <ScreenHeader title={t("projects.title")} noBorder />
        ...
        ListEmptyComponent={() => (
          <View style={s.empty}>
            ...
            <Text style={[s.emptyText, { color: c.mutedForeground }]}>
              {isLoading ? t("notifications_screen.loading") : t("projects.no_projects")}
            </Text>
          </View>
        )}
        renderItem={({ item }) => (
          ...
          >
            ...
            <View style={[s.badge, {
              backgroundColor: item.isActive ? `${c.success}18` : `${c.mutedForeground}18`
            }]}>
              <Text style={[s.badgeTxt, { color: item.isActive ? c.success : c.mutedForeground }]}>
...
function makeStyles(c: ReturnType<typeof useColors>["colors"]) {
  return StyleSheet.create({
    ...
    card:       { flexDirection: "row", alignItems: "center", gap: 12, borderRadius: 14, padding: 14, marginBottom: 10, borderWidth: 1, ...shadow({ opacity: 0.05, radius: 8, elevation: 2 }) },
