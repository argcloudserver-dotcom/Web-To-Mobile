...
const STATUSES = ["new", "called", "qualified", "meeting", "negotiation", "bought", "not_qualified", "low_budget"] as const;
          ...
          {showNoteInput ? (
            <View style={{ gap: 8 }}>
              ...
              <View style={{ flexDirection: "row", gap: 8, justifyContent: "flex-end" }}>
                ...
                <TouchableOpacity
                  style={[s.logBtn, { backgroundColor: c.accent }, createActivity.isPending && { opacity: 0.6 }]}
                  onPress={handleLogCall}
                  disabled={createActivity.isPending}
                >
                  <Feather name="phone" size={14} color={c.accentForeground} />
                  <Text style={[s.logBtnText, { color: c.accentForeground }]}>Log Call</Text>
                </TouchableOpacity>
              </View>
            </View>
          ) : (
...
function makeStyles(c: ReturnType<typeof useColors>["colors"]) {
  return StyleSheet.create({
    ...
    card:          { backgroundColor: c.card, borderRadius: 16, padding: 20, borderWidth: 1, borderColor: c.border, marginBottom: 20, ...shadow({ opacity: 0.06, radius: 10, elevation: 3 }) },
    ...
    logBtn:        { flexDirection: "row", alignItems: "center", gap: 6, paddingHorizontal: 16, paddingVertical: 8, borderRadius: 8 },
