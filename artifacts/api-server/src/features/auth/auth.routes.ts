...
/**
 * Determine whether the caller is the mobile app. Body `client` wins
 * because that's the schema field; when it's absent (generated
 * `LoginInput` type has no `client` field, for example), fall back to the
 * `X-Client-Platform` header the mobile bundle sets at boot.
 */
function isMobileClient(req: { body: any; headers: any }): boolean {
  if (req.body?.client === "mobile") return true;
...
router.post("/auth/login", validateBody(loginBody), asyncHandler(async (req, res) => {
  ...
  if (isMobileClient(req)) return ok(res, { user: sanitizeUser(user), token, accessToken: token });
