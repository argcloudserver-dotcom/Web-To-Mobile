...
export async function loadDailyTrends(params: TrendsParams): Promise<TrendsRow[]> {
  ...
  const query = sql`
    SELECT
      ...
      COUNT(*)::int AS total,
      COUNT(CASE WHEN status = 'bought' THEN 1 END)::int AS won,
      COUNT(CASE WHEN status IN ('not_qualified','low_budget') THEN 1 END)::int AS lost,
      COUNT(CASE WHEN status NOT IN ('bought','not_qualified','low_budget') THEN 1 END)::int AS in_progress
    FROM leads
...
export async function loadProjectsReport(
  ...
  const query = sql`
    SELECT
      ...
      COUNT(CASE WHEN l.status = 'bought' THEN 1 END)::int AS won,
      COUNT(CASE WHEN l.status IN ('not_qualified','low_budget') THEN 1 END)::int AS lost,
      COUNT(CASE WHEN l.status NOT IN ('bought','not_qualified','low_budget') THEN 1 END)::int AS in_progress
    FROM projects p
