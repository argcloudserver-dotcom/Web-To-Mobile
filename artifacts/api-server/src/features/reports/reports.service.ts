...
export async function buildSalesReport(query: ReportsQuery): Promise<SalesReport> {
  ...
  for (const l of leads) {
    ...
    if (l.status === "bought") stats[sid].won++;
    else if ((l.status === "not_qualified" || l.status === "low_budget")) stats[sid].lost++;
  ...
  return {
    totalWon: leads.filter((l) => l.status === "bought").length,
    totalLost: leads.filter((l) => (l.status === "not_qualified" || l.status === "low_budget")).length,
