...
export async function getStats(req: Request): Promise<DashboardStats> {
  ...
  return {
    ...
    activeLeads: leads.filter((l) => !["bought", "not_qualified", "low_budget"].includes(l.status)).length,
    wonLeads: leads.filter((l) => l.status === "bought").length,
    lostLeads: leads.filter((l) => (l.status === "not_qualified" || l.status === "low_budget")).length,
...
export async function getTopPerformers(req: Request): Promise<TopPerformer[]> {
  ...
  for (const l of leads) {
    ...
    if (l.status === "bought") stats[l.salesId].won++;
  }
