...
// FIX: deactivate all active assignments for a lead (admin/ceo/director only).
import { and } from "drizzle-orm";
export async function deactivateActiveAssignments(leadId: string) {
  return db
    .update(leadAssignmentsTable)
    .set({ isActive: false, unassignedAt: new Date() })
    .where(
      and(
        eq(leadAssignmentsTable.leadId, leadId),
        eq(leadAssignmentsTable.isActive, true),
      ),
    )
    .returning();
}

// Deactivate a SINGLE active assignment (per-user unassign).
export async function deactivateAssignmentForUser(leadId: string, userId: string) {
  return db
    .update(leadAssignmentsTable)
    .set({ isActive: false, unassignedAt: new Date() })
    .where(
      and(
        eq(leadAssignmentsTable.leadId, leadId),
        eq(leadAssignmentsTable.assignedTo, userId),
        eq(leadAssignmentsTable.isActive, true),
      ),
    )
    .returning();
...
// Active-only assignments for a lead (used to dedupe in multiAssign).
export async function findActiveAssignmentsForLead(leadId: string) {
  return db
    .select()
    .from(leadAssignmentsTable)
    .where(
