...
export async function getLead(
  ...
  if (visibleIds !== null) {
    ...
    // Primary path: the lead's owner is within the viewer's visible scope.
    if (owner && visibleIds.includes(owner)) return enrich(row);
    // Fallback: viewer is directly assigned to this lead (e.g. a team leader
    // or sales rep whose assignment record exists before primarySalesId is set,
    // or when the lead has multiple assignees but a different primary).
    const activeAssignments = await repo.findActiveAssignmentsForLead(leadId);
    const isAssigned = activeAssignments.some(
      (a) => a.assignedTo === req.currentUser!.id,
    );
...
async function notifyStatusChange(
  ...
  try {
    ...
    repo
      .insertNotification({
        userId: salesUser.id,
        type: newStatus === "bought" ? "lead_won" : "lead_status_changed",
        titleEn: newStatus === "bought" ? "Deal Closed! 🎉" : "Lead Status Updated",
...
export async function multiAssignLead(
  ...
  // Dedupe targets by (userId+role) — UI may submit duplicates if a user
  // double-clicks. Without this we'd write duplicate "active" rows.
  const seen = new Set<string>();
  const dedupedTargets = input.targets.filter((t) => {
    const key = `${t.userId}:${t.role}`;
  ...
  const targetIds = [...new Set(dedupedTargets.map((t) => t.userId))];
  ...
  // FIX: support transfer/reassign — deactivate prior active assignments first.
  if (input.replace === true) {
    await repo.deactivateActiveAssignments(leadId);
  }

  // FIX: when NOT replacing, skip targets that are already actively assigned.
  // Prevents the "same sales appears twice in Currently Assigned" bug.
  let newTargets = dedupedTargets;
  let skipped = 0;
  if (input.replace !== true) {
    const active = await repo.findActiveAssignmentsForLead(leadId);
    const activeIds = new Set(active.map((a) => a.assignedTo));
    newTargets = dedupedTargets.filter((t) => {
      if (activeIds.has(t.userId)) {
        skipped += 1;
        return false;
      }
      return true;
  ...
  if (newTargets.length === 0) {
    return { count: 0, skipped, assignments: [], alreadyAssigned: true as const };
  }
  ...
  const inserted = await repo.insertAssignments(
    newTargets.map((t) => ({
      ...
      assignmentType: (input.replace ? "reassign" : "assign") as "assign",
  ...
  const firstSales = newTargets.find((t) => t.role === "sales");
  ...
  return { count: inserted.length, skipped, assignments: inserted };
}
...
// FIX: unassign all current assignees from a lead and clear primarySalesId.
// Allowed roles enforced at the route layer (admin/ceo/director).
export async function unassignAllForLead(
  req: Request,
  leadId: string,
) {
  const viewer = req.currentUser!;
  ...
  const deactivated = await repo.deactivateActiveAssignments(leadId);
  await repo.updateById(leadId, { primarySalesId: null });
  ...
  return { count: deactivated.length };
}

// FIX: unassign a single user from a lead. If they were primarySales,
// promote the next active sales (if any) to primary, else clear it.
export async function unassignUserFromLead(
  req: Request,
  leadId: string,
  userId: string,
) {
  ...
  const deactivated = await repo.deactivateAssignmentForUser(leadId, userId);
  if (deactivated.length === 0) {
    return { count: 0, promotedPrimary: null as string | null };
  }

  let promotedPrimary: string | null = null;
  if (lead.primarySalesId === userId) {
    const remaining = await repo.findActiveAssignmentsForLead(leadId);
    const nextSales = remaining.find((a) => a.assignedToRole === "sales");
    promotedPrimary = nextSales?.assignedTo ?? null;
    await repo.updateById(leadId, { primarySalesId: promotedPrimary });
  }
  ...
  try {
    await repo.insertAssignment({
      leadId,
      assignedTo: userId,
      assignedToRole: deactivated[0].assignedToRole,
      assignedBy: viewer.id,
      assignmentType: "unassign",
      note: "Removed by " + viewer.role,
      isActive: false,
    });
  } catch {
    /* best-effort audit */
  ...
  return { count: deactivated.length, promotedPrimary };
}
