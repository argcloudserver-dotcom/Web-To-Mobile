...
// FIX: DELETE /leads/:leadId/assignments — unassign all current assignees.
// Frontend AssignLeadModal "Unassign lead" action calls this endpoint;
// without it the request returned 404.
router.delete(
  "/leads/:leadId/assignments",
  requireAuth,
  withPermission("leads.assign"),
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!role || !["admin", "ceo", "director"].includes(role)) {
      return fail(res, 403, {
        code: "FORBIDDEN",
        message: "Only Admin, CEO or Director can unassign a lead",
      });
    }
    const result = await service.unassignAllForLead(req, req.params.leadId);
    if (!result) {
      return fail(res, 404, { code: "NOT_FOUND", message: "Lead not found" });
    }
    return ok(res, result);
...
// FIX: DELETE /leads/:leadId/assignments/:userId — remove a SINGLE assignee.
// Allowed roles: admin, ceo, director, team_leader (TL can only unassign from
// their own team — enforced by service via getVisibleUserIds when needed).
router.delete(
  "/leads/:leadId/assignments/:userId",
  requireAuth,
  withPermission("leads.assign"),
  validateParams(leadAssigneeParams),
  asyncHandler(async (req, res) => {
    const role = req.currentUser?.role;
    if (!role || !["admin", "ceo", "director", "team_leader"].includes(role)) {
      return fail(res, 403, {
        code: "FORBIDDEN",
        message: "You don't have permission to unassign users",
      });
    }
    const { leadId, userId } = req.params;
    const result = await service.unassignUserFromLead(req, leadId, userId);
    if (!result) {
      return fail(res, 404, { code: "NOT_FOUND", message: "Lead not found" });
    }
    return ok(res, result);
